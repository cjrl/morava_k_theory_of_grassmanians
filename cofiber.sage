...
# degree = preimage_degree_for_Qn(2*(c+1),n)
# possible_pairs =[x for x in g.H[degree]]
# print possible_pairs  
# # print [h.Q(n)(x) for x in possible_pairs]
...
# print "a=",a
# print "b=",b
# print "Qn(w1^aw2^b) = ", h.Q(n)(g.W[1]^a*g.W[2]^b)
# expansion = expand_in_terms_of_groebner(h.W[1]^(a+(2^(n+1)-1))*h.W[2]^b,h.ideal)
...
#                print h.Q(n)(beta(i,h))
#                print h.W[1]^(2*(i+1))*h.wbar_expression_in_w(c+1)
...
#      c = m-2
#      h = Gr(2,c)
#      print (h.Q(n)(h.W[1]*h.wbar_expression_in_w(s)))
#      print (h.W[1]*h.wbar_expression_in_w(s+2^(n+1)-1))

#      print h.W[1]*h.Q(n)(h.wbar_expression_in_w(s))
#      # print h.Q(n)(h.W[1])*h.wbar_expression_in_w(s)

# wbar_conj(2,3,2)

def wbar_without_coeff(r):
     expontents = []
     for a in range(0,r+1):
          for b in range(0,ceil(r/2)+1):
               if a+2*b == r:
                    expontents.append([a,b])
     return expontents


# for b in range(2*l+1,(c+2*l+2)/2+1):
#      print binomial(c+1-b,b-(2*l+1)) % 2,  c+2*l-2*b+2, b

# def p2(k):
#     return len([x for x in Partitions(k).list() if len(x) == 2])

# exp = 0
# for k in range(0,2^n+1):
#      exp += (k+1)*g.W[1]^(2^(n+1)-2*k)*g.W[2]^(k+2*l)
# print exp
...
#          exp += binomial(2^(n)-2*i+i,2^(n)-2*i)*binomial(2^(n)-2*j+j,2^(n)-2*j)*g.W[1]^(2^(n+1)-2*i-2*j)*g.W[2]^(i+j)
# print exp

# exp =0
# for i in range(0,2^(n-1)+1):
#          exp += binomial(2^(n)-2*i+i,2^(n)-2*i)*g.W[1]^(2^(n)-2*i)*g.W[2]^(i)
# print exp
...
#          exp += binomial(2^(n)-2*i+i,2^(n)-2*i)^2*g.W[1]^(2^(n+1)-4*i)*g.W[2]^(2*i)
# print exp

# exp = 0
# for i in range(1,2^n+1):
#      # print binomial(2^(n)-2*i+i,2^(n)-2*i)^2 % 2
#      exp += binomial(2^(n)-2*i+i,2^(n)-2*i)^2*g.W[1]^(2^(n+1)-4*i)*g.W[2]^(2*i)*g.W[2]^(2*l)
# print exp

# for b in range(2*l+2,(c+2*l+2)/2+1):
#      print b,binomial(c+1-b,b-(2*l+1)) % 2

# exp = 0
# for b in range(1,2^(n-1)+1):
#      # print binomial(2^(n)-2*i+i,2^(n)-2*i)^2 % 2
#      exp += g.W[1]^(2^(n+1)+4*l-2*b)*g.W[2]^(2*b+l)
#      print exp
# print exp

# def gb_test(c,n,l):     
#     exp = 0
#     for b in range(2*l+1,2^n+2*l+1):
#          exp += binomial(c+1-b,b-2*l-1)*g.W[1]^(2^(n+1)+4*l-2*b)*g.W[2]^b
#     return exp


# for b in range(1,2^(n-1)+1):
#      # print b,binomial(2^(n+1)-2*b-1,2*b-1) % 2
#      # print b,binomial(2^(n)-b,2^n-2*b)^2 % 2
#      # print binomial(2^(n+1)-2*b-1,2*b-1) % 2, binomial(2^(n+1)-2*b-1,2*b-1) % 2
#      # print binomial(2^n-b,2^n-2*b)^2 % 2 
#      print binomial(2^(2+1)-2*b-1,2*b)
...
#      # print 2^n-b
#      # print 2^n-2*b
#      # print (binomial(2^n-b,2^n-2*b))^2 % 2, binomial(2^(n+1)-2*b+1,2*b-1)%2
#      # print binomial(2^(n+1)-2*b,2*b)%2 == binomial(2^(n+1)-2*b-1,2*b-2)%2

# n=6
# for x in range(0,2^(n)):
#      b = 2*x +1
#      print b,binomial(2^(n+1) -b -1,b-1)%2

def sum_test():
     exp = 0
     for i in range(2,n+1):
          exp += h.W[1]^(2^(n+1)-2^(i+1)+1)*h.W[2]^(2^i)
     return exp
          

# for b in range(0,2^n+l+1):
#      print b,binomial(m^(n+1)+2*l-b,b)%2
...
# print "Qn",h.Q(n)(h.W[1]^a*h.W[2]^b)

# print "w2^(b-1)*(wbar(d)+w1^d) expansion:"
# expansion = expand_in_terms_of_groebner(h.W[1]^a*h.W[2]^(b-1)*(h.wbar_expression_in_w(d)+h.W[1]^d),h.ideal)
# expansion = expand_in_terms_of_groebner(h.Q(n)(h.W[1]^a*h.W[2]^(2*l+1)),h.ideal)
expansion = expand_in_terms_of_groebner(h.W[1]^(2^(n+1))*h.W[2]^(2*l+2),h.ideal)
...
# for b in range(0,2^(n)-2+1):
#      print b,binomial(m-1-b,b-(4+2*l))%2

# def g0_conj():
#      exp = h.W[1]^m
#      for i in range(2,n+1):
#           exp += h.W[1]^(2^(n+1)+3-2^(i+1))*h.W[2]^(2^i)
#      return exp
          
# def g_sq_conj():
#      exp = 0
#      for b in range(2^(n+1),(2^(n+1)+1)+1):
#           exp += h.W[1]^(2^(n+2)+3-2*b)*h.W[2]^b
#      return exp

# for b in range(1,2^(n-1)+1):
#      print b, binomial(2^(n+1)-2*b-1,2*b-2)%2, binomial(2^n-b,2^n-2*b)^2%2

for b in range(2*l+2,2^n+2*l+1+1):
     print b, binomial(2^(n+1)+2*l+1-b,b-2*l-2)%2
